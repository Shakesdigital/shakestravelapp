...
export default function RegisterPage() {
  ...
  const { user: netlifyUser, signup: netlifySignup, loading: netlifyLoading } = useNetlifyIdentity();
  const router = useRouter();
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);
  
  // Handle Netlify Identity registration success
  useEffect(() => {
    if (netlifyUser && !netlifyLoading) {
      // Redirect to profile or onboarding after successful registration
      router.push('/profile');
    }
  }, [netlifyUser, netlifyLoading, router]);
        ...
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          ...
          <div className="mt-6">
            ...
            <div className="mt-6 space-y-3">
              ...
              <button
                type="button"
                onClick={netlifySignup}
                disabled={netlifyLoading || loading}
                className="w-full flex justify-center items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 disabled:bg-gray-100 disabled:cursor-not-allowed"
              >
                ...
                {netlifyLoading ? 'Loading...' : 'Sign up with Google (Netlify)'}
              </button>

              {/* Original Google OAuth (fallback) */}
              <GoogleSignInButton
                onSuccess={handleGoogleSuccess}
                onError={handleGoogleError}
                disabled={loading || netlifyLoading}
                className="w-full"
              />
            </div>
          </div>
