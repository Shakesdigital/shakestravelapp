...
export default function LoginPage() {
  const { login } = useAuth();
  const { user: netlifyUser, login: netlifyLogin, loading: netlifyLoading } = useNetlifyIdentity();
  const router = useRouter();
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);
  
  // Handle Netlify Identity login success
  useEffect(() => {
          ...
          <div className="mt-6">
            ...
            <div className="mt-6 space-y-3">
              ...
              >
                ...
                {netlifyLoading ? 'Loading...' : 'Sign in with Google (Netlify)'}
              </button>
