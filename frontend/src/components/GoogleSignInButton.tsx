...
const GoogleSignInButton: React.FC<GoogleSignInButtonProps> = ({
  ...
  useEffect(() => {
    ...
    if (googleClientId && googleClientId !== 'demo-client-id') {
      // Load Google Identity Services script for production
  ...
  // Fallback manual button for when Google SDK is not available
  const handleManualGoogleSignIn = async () => {
    try {
      ...
      const fullCredential = `header.${mockCredential}.signature`;
      await googleLogin(fullCredential);
      onSuccess?.();
    } catch (error) {
  ...
  const googleClientId = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID;
  const hasValidGoogleConfig = googleClientId && googleClientId !== 'demo-client-id';

  return (
    <div className={`google-signin-container ${className}`}>
      {hasValidGoogleConfig ? (
        // Google's official sign-in button will render here when properly configured
        <div id="google-signin-button" style={{ minHeight: '40px' }}></div>
      ) : (
        // Demo button for development/testing when Google OAuth is not configured
        <button
